...
const NavBar = () => {
  ...
  return (
    <nav className={`w-full fixed top-0 transition-colors z-50 duration-300 ${isAuthPage ? 'bg-white shadow-md' : isScrolled ? 'bg-[#051036]' : ''}`}>
      <div className="w-[95%] mx-auto flex items-center justify-between py-4"> 
          ...
          <button className={"bg-white text-[#0D1B2A] px-8 py-3 rounded-sm  hover:bg-gray-100" }> Become An Expert </button>
            <button className={`md:block hidden border px-8 py-3 rounded-sm  'border-black' : "border-white" } `}> <NavLink  to="/login">Sign In / Register</NavLink></button>
